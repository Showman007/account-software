...
const columns: GridColDef[] = [
  ...
  { field: 'party_type', headerName: 'Type', width: 100 },
  {
    ...
    width: 150,
    renderCell: (params) => formatINR(params.value as number),
  },
...
export default function PartiesPage() {
  ...
  const defaults = useMemo(() => {
    if (editing) {
      return {
        name: editing.name,
        village_city: editing.village_city,
        phone: editing.phone,
  ...
  const handleDelete = (row: Party) => {
    if (window.confirm(`Delete party "${row.name}"?`)) {
      crud.deleteMutation.mutate(row.id);
    }
  };
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) {
      crud.updateMutation.mutate(
        { id: editing.id, data: data as Partial<Party> },
        { onSuccess: () => setDialogOpen(false) }
      );
    } else {
      crud.createMutation.mutate(data as Partial<Party>, {
        onSuccess: () => setDialogOpen(false),
      });
    }
  };
