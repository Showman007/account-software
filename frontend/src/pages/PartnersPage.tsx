...
const columns: GridColDef[] = [
  ...
  { field: 'profit_share_rate', headerName: 'Share Rate', width: 110 },
  {
    ...
    renderCell: (p) => (
      <Chip
        label={(p.value as string).toUpperCase()}
...
export default function PartnersPage() {
  ...
  const defaults = useMemo(() => {
    if (editing) {
      return {
        ...
        profit_share_type: editing.profit_share_type,
        profit_share_rate: editing.profit_share_rate,
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) {
      crud.updateMutation.mutate({ id: editing.id, data: data as Partial<Partner> }, { onSuccess: () => setDialogOpen(false) });
    } else {
      crud.createMutation.mutate(data as Partial<Partner>, { onSuccess: () => setDialogOpen(false) });
    }
  };
