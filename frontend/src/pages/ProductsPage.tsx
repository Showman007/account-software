...
export default function ProductsPage() {
  ...
  const defaults = useMemo(() => {
    if (editing) {
      return { name: editing.name, category: editing.category, direction: editing.direction, default_unit_id: editing.default_unit_id };
    }
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) {
      crud.updateMutation.mutate({ id: editing.id, data: data as Partial<Product> }, { onSuccess: () => setDialogOpen(false) });
    } else {
      crud.createMutation.mutate(data as Partial<Product>, { onSuccess: () => setDialogOpen(false) });
    }
  };
