...
const PaymentModesPageComp = () => {
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) crud.updateMutation.mutate({ id: editing.id, data: data as Partial<PaymentMode> }, { onSuccess: () => setDialogOpen(false) });
    else crud.createMutation.mutate(data as Partial<PaymentMode>, { onSuccess: () => setDialogOpen(false) });
  };
  return (
    <>
      <DataTable title="Payment Modes" columns={columns} rows={crud.data} loading={crud.isLoading}
      ...
      {dialogOpen && (
        <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSubmit={handleSubmit}
          title={editing ? 'Edit Payment Mode' : 'Add Payment Mode'} defaultValues={defaults}
...
export default PaymentModesPageComp;
