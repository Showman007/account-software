...
const InboundPageComp = () => {
  ...
  const defaults = useMemo(() => {
    ...
    return { date: new Date().toISOString().slice(0, 10), bag_type: '', no_of_bags: '', qty: 0, rate: 0, moisture_pct: 0, paid: 0, category: '' };
  }, [editing]);
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) {
      crud.updateMutation.mutate({ id: editing.id, data: data as Partial<InboundEntry> }, {
        onSuccess: () => {
          const file = stagedFileRef.current;
          if (file && editing.id) {
            uploadAttachment('inbound_entries', editing.id, file).catch(() => {});
          }
          setStagedFile(null);
  ...
  const tableComp = () => (
    <DataTable
      ...
      mobileHiddenColumns={['id', 'village', 'category', 'bag_type', 'no_of_bags', 'qty', 'unit_id', 'rate', 'gross_amt', 'moisture_pct', 'paid']}
    ...
    <>
      {dialogOpen && (
        ...
        >
          ...
          <BagQuantityFields showAmount amountLabel="Gross Amount" unitMap={unitMap} />
