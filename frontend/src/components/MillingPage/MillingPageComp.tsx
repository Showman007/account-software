...
import { FormField, FormDateField } from '../common/FormField.tsx';
...
const MillingPageComp = () => {
  ...
  const defaults = useMemo(() => {
    if (editing) {
      return { date: editing.date, paddy_type: editing.paddy_type, miller_name: editing.miller_name, input_qty: editing.input_qty, milling_cost: editing.milling_cost, rice_main_qty: editing.rice_main_qty, broken_rice_qty: editing.broken_rice_qty, rice_bran_qty: editing.rice_bran_qty, husk_qty: editing.husk_qty, rice_flour_qty: editing.rice_flour_qty };
    }
    ...
    <>
      ...
      {dialogOpen && (
        <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSubmit={handleSubmit}
          title={editing ? 'Edit Milling Batch' : 'Add Milling Batch'} defaultValues={defaults}
...
export default MillingPageComp;
