...
const CreditTransactionsPageComp = () => {
  ...
  const defaults = useMemo(() => {
    if (editing) return { date: editing.date, partner_id: editing.partner_id, transaction_type: editing.transaction_type, credit_received: editing.credit_received, principal_returned: editing.principal_returned, profit_paid: editing.profit_paid, payment_mode_id: editing.payment_mode_id, used_for: editing.used_for, remarks: editing.remarks };
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) crud.updateMutation.mutate({ id: editing.id, data: data as Partial<CreditTransaction> }, { onSuccess: () => setDialogOpen(false) });
    else crud.createMutation.mutate(data as Partial<CreditTransaction>, { onSuccess: () => setDialogOpen(false) });
  };
    ...
    <>
      ...
      {dialogOpen && (
        <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSubmit={handleSubmit}
          title={editing ? 'Edit Credit Transaction' : 'Add Credit Transaction'} defaultValues={defaults}
...
export default CreditTransactionsPageComp;
