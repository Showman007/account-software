...
import type { Unit } from '../../types/masters.ts';
...
const UnitsPageComp = () => {
  ...
  const defaults = useMemo(() => { if (editing) return { name: editing.name, abbreviation: editing.abbreviation }; return {}; }, [editing]);
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) crud.updateMutation.mutate({ id: editing.id, data: data as Partial<Unit> }, { onSuccess: () => setDialogOpen(false) });
    else crud.createMutation.mutate(data as Partial<Unit>, { onSuccess: () => setDialogOpen(false) });
  };
  return (
    <>
      <DataTable title="Units" columns={columns} rows={crud.data} loading={crud.isLoading}
      ...
      {dialogOpen && (
        <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSubmit={handleSubmit}
          title={editing ? 'Edit Unit' : 'Add Unit'} defaultValues={defaults}
