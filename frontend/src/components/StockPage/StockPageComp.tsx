...
const StockPageComp = () => {
  ...
  const defaults = useMemo(() => {
    if (editing) return { opening_stock: editing.opening_stock, min_level: editing.min_level };
    return { opening_stock: 0, min_level: 0 };
  }, [editing]);

  const handleRecalculate = async () => {
    setRecalculating(true);
    try { await recalculateStock(); toast.success('Stock recalculated successfully'); }
    catch { toast.error('Failed to recalculate stock'); }
    finally { setRecalculating(false); }
  };

  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) crud.updateMutation.mutate({ id: editing.id, data: data as Partial<StockItem> }, { onSuccess: () => setDialogOpen(false) });
  };
    ...
    <>
      ...
      {dialogOpen && editing && (
        <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSubmit={handleSubmit}
          title="Edit Stock Item" defaultValues={defaults} isLoading={crud.updateMutation.isPending}>
